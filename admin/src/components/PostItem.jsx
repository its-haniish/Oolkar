...
const PostItem = ({ slug, setBlogs, index }) => {
    ...
    const deletePost = async (slug) => {
        ...
        try {
            let res = await fetch(`${process.env.REACT_APP_BASE_URL}/delete-post`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ slug })
            })
            let result = await res.json()
            if (result?.msg === "Post deleted successfully.") {
                setBlogs(prev => prev.filter(elem => elem.slug !== slug))
            }
        } catch (error) {
            alert(error);
        }
